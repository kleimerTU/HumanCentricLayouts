...
def main(argv):
  ...
  for i in range(n_sets):
        ...
        for furn in furniture:
          ...
          if furn.coarse_category == "window" or furn.coarse_category == "door":
            ...
            if (rot > 0.025*np.pi) and (rot < 0.475*np.pi): # check if axis aligned
              all_aligned = False
        if all_aligned:
          room_seq = create_sequence(furniture,dict_cat2int,detailed=True,keep_room_dims=False,res=res)
    ...
    minpos = torch.min(torch.stack([minvalue_dict[i][1:3] for i in range(len(categories))],0),0)[0]
    maxpos = torch.max(torch.stack([maxvalue_dict[i][1:3] for i in range(len(categories))],0),0)[0]
    for i in range(len(categories)):
      minvalue_dict[i][1:3] = minpos.flatten()
      maxvalue_dict[i][1:3] = maxpos.flatten()
    ...
    for i in range(len(train_sequences)):
      if len(train_sequences[i]) > 0:
        ...
        quantized = quantize_sequence_grid(train_sequences[i][i_sort,:],minvalue_dict,maxvalue_dict,res)
    ...
    for i in range(len(val_sequences)): 
      if len(val_sequences[i]) > 0: 
        ...
        quantized = quantize_sequence_grid(val_sequences[i][i_sort,:],minvalue_dict,maxvalue_dict,res)
    ...
    ergo_scores_new_train = evaluate_scenes(new_sequences_train, minvalue_dict, maxvalue_dict, dict_cat2fun, res=res, use_log=False,grid_quantization=True, device='cpu')
    pickle.dump(ergo_scores_new_train, open(path_input_data + set_name + "_data/" + set_name + "_ergo_scores_train" + suffix_name + ".pkl", "wb"))
    ergo_scores_new_val = evaluate_scenes(new_sequences_val, minvalue_dict, maxvalue_dict, dict_cat2fun, res=res, use_log=False,grid_quantization=True, device='cpu')
  ...
  if args.num_room_plots > 0:
    ...
    for j in range(args.num_room_plots):
      ...
      unquantized = unquantize_sequence_grid(quantized,minvalue_dict,maxvalue_dict,res)
