...
if config.continue_train:
    encoder = torch.load(config.load_encoder)
    decoder = torch.load(config.load_decoder)
else:
...
if config.no_verbose:
    scheduler_enc = torch.optim.lr_scheduler.StepLR(encoder_opt, step_size=config.lr_decay_every, gamma=config.lr_decay_by)
    scheduler_dec = torch.optim.lr_scheduler.StepLR(decoder_opt, step_size=config.lr_decay_every, gamma=config.lr_decay_by)
else:
    scheduler_enc = torch.optim.lr_scheduler.StepLR(encoder_opt, step_size=config.lr_decay_every, gamma=config.lr_decay_by, verbose=True)
    scheduler_dec = torch.optim.lr_scheduler.StepLR(decoder_opt, step_size=config.lr_decay_every, gamma=config.lr_decay_by, verbose=True)
